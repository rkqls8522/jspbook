...
<HTML>
<HEAD><TITLE>BBS(검색모드)</TITLE>
</HEAD>
<BODY>
<P>
<P align=center><img src=image/bbs.jpg></P> 
<FORM method=post name=search action="bbs_search.jsp">
...
try {
 ...
 String sql = "select * from bbs " + cond;
 ...
 if (!(rs.next()))  {
    ...
    out.println("<TD>");
          ...
          String clink = "<A href=bbs_read.jsp?id=" + keyid.elementAt(j) ;
...
if (wheregroup > 1) {
      out.print("[<A href=bbs_search.jsp?gogroup=1"); 
      out.print("&stype="+ what+"&sval=" + val +">처음</A>]");

      out.print("[<A href=bbs_search.jsp?gogroup="+priorgroup);
...
if (name.size() !=0) {
  for(int jj=startpage; jj<=endpage; jj++) {
    if (jj==where) 
       ...
       out.print("[<A href=bbs_search.jsp?go="+jj);
       out.print("&stype="+ what+"&sval=" + val +">" + jj + "</A>]") ;
     }
  }
}
if (wheregroup < totalgroup) {
       out.print("[<A href=bbs_search.jsp?gogroup="+ nextgroup);
       out.print("&stype="+ what+"&sval=" + val +">다음</A>]");
   
       out.print("[<A href=bbs_search.jsp?gogroup="+totalgroup); 
...
%>
<p align=center>[<A href="bbs_list.jsp">BBS(초기화면)</A>]
</BODY>
</HTML>
